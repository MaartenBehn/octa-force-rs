...
impl MainGui {
    pub(crate) fn new(context: &vulkan::Context, command_pool: &CommandPool, window: &Window, format: Format, in_flight_frames: usize) -> Result<Self> {
    ...
    pub(crate) fn render<G: Gui>(&mut self, buffer: &CommandBuffer, gui: &mut G, frame_stats: &mut FrameStats, window: &Window, extent: Extent2D) -> Result<()> {
        let mut imgui = self.imgui_context.take().unwrap().activate().unwrap();
